...
public class Block implements ITextureProvider
{
    ...
    public static boolean isNormalCube(int par0)
    {
        ...
        return block != null ? block.blockMaterial.isOpaque() && block.renderAsNormalBlock() : false;
    }
    ...
    {
        return par5 != 0 || minY <= 0.0D ? par5 != 1 || maxY >= 1.0D ? par5 != 2 || minZ <= 0.0D ? par5 != 3 || maxZ >= 1.0D ? par5 != 4 || minX <= 0.0D ? par5 != 5 || maxX >= 1.0D ? !par1IBlockAccess.isBlockOpaqueCube(par2, par3, par4) : true : true : true : true : true : true;
    }
    ...
    public void dropBlockAsItemWithChance(World par1World, int par2, int par3, int par4, int par5, float par6, int par7)
    {
        if (!par1World.isRemote)
        {
            ...
            ArrayList arraylist = getBlockDropped(par1World, par2, par3, par4, par5, par7);
    ...
    {
        return par1Vec3D != null ? par1Vec3D.yCoord >= minY && par1Vec3D.yCoord <= maxY && par1Vec3D.zCoord >= minZ && par1Vec3D.zCoord <= maxZ : false;
    }
    ...
    {
        return par1Vec3D != null ? par1Vec3D.xCoord >= minX && par1Vec3D.xCoord <= maxX && par1Vec3D.zCoord >= minZ && par1Vec3D.zCoord <= maxZ : false;
    }
    ...
    {
        return par1Vec3D != null ? par1Vec3D.xCoord >= minX && par1Vec3D.xCoord <= maxX && par1Vec3D.yCoord >= minY && par1Vec3D.yCoord <= maxY : false;
    }
    ...
    public boolean isBlockSolidOnSide(World world, int i, int j, int k, int l)
    {
        ...
        {
            return (i1 & 8) == 8 && l == 1 || isOpaqueCube();
        }
    ...
    public float blockStrength(World world, EntityPlayer entityplayer, int i, int j, int k)
    {
        return blockStrength(entityplayer, world.getBlockMetadata(i, j, k));
    }

    public float blockStrength(EntityPlayer entityplayer, int i)
    {
        return ForgeHooks.blockStrength(this, entityplayer, i);
    }
    ...
    public boolean removeBlockByPlayer(World world, EntityPlayer entityplayer, int i, int j, int k)
    {
    ...
    {
        return getFlammability(iblockaccess, i, j, k, l, i1) > 0;
    }
    ...
    {
        if (blockID == netherrack.blockID && i1 == 0)
        {
            return true;
        }

        return (world.worldProvider instanceof WorldProviderEnd) && blockID == bedrock.blockID && i1 == 0;
    }

    public static void setBurnProperties(int i, int j, int k)
    {
        blockFireSpreadSpeed[i] = j;
        blockFlammability[i] = k;
    }
    ...
    public TileEntity getTileEntity(int i)
    {
        ...
        {
            return ((BlockContainer)this).getBlockEntity(i);
        }
        else
        {
            return null;
    ...
    {
        return quantityDroppedWithBonus(j, random);
    }

    public ArrayList getBlockDropped(World world, int i, int j, int k, int l, int i1)
    {
        ArrayList arraylist = new ArrayList();
        int j1 = quantityDropped(l, i1, world.rand);

        for (int k1 = 0; k1 < j1; k1++)
        {
            int l1 = idDropped(l, world.rand, 0);

            if (l1 > 0)
            {
                arraylist.add(new ItemStack(l1, 1, damageDropped(l)));
            }
        }
    ...
    public boolean canSilkHarvest(World world, EntityPlayer entityplayer, int i, int j, int k, int l)
    {
        ...
        {
            return renderAsNormalBlock() && !hasTileEntity(l);
        }
    }

    public boolean canCreatureSpawn(EnumCreatureType enumcreaturetype, World world, int i, int j, int k)
    {
        ...
        if (this instanceof BlockStep)
        {
            ...
            {
                return (l & 8) == 8 || isOpaqueCube();
            }
            else
            {
                return isNormalCube(blockID);
            }
        }
        ...
        {
            return isBlockSolidOnSide(world, i, j, k, 1);
        }
    }

    public boolean isBed(World world, int i, int j, int k, EntityLiving entityliving)
    {
        return blockID == bed.blockID;
    }

    public ChunkCoordinates getBedSpawnPosition(World world, int i, int j, int k, EntityPlayer entityplayer)
    {
        return BlockBed.getNearestEmptyChunkCoordinates(world, i, j, k, 0);
    }

    public void setBedOccupied(World world, int i, int j, int k, EntityPlayer entityplayer, boolean flag)
    {
        BlockBed.setBedOccupied(world, i, j, k, flag);
    }

    public int getBedDirection(IBlockAccess iblockaccess, int i, int j, int k)
    {
        return BlockBed.getDirection(iblockaccess.getBlockMetadata(i, j, k));
    }

    public boolean isBedFoot(IBlockAccess iblockaccess, int i, int j, int k)
    {
        return BlockBed.isBlockFootOfBed(iblockaccess.getBlockMetadata(i, j, k));
    }
    ...
    public boolean canBeReplacedByLeaves(World world, int i, int j, int k)
    {
    ...
    public boolean isGenMineableReplaceable(World world, int i, int j, int k)
    {
    ...
    static
    {
        ...
        blockFireSpreadSpeed = new int[blocksList.length];
        blockFlammability = new int[blocksList.length];
