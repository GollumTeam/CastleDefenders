...
public class BlockLeaves extends BlockLeavesBase implements IShearable
{
    ...
    {
        return (par1 & 3) != 1 ? (par1 & 3) != 2 ? ColorizerFoliage.getFoliageColorBasic() : ColorizerFoliage.getFoliageColorBirch() : ColorizerFoliage.getFoliageColorPine();
    }
            ...
            for (int k = -i; k <= i; k++)
            {
                for (int l = -i; l <= i; l++)
                {
                    for (int i1 = -i; i1 <= i; i1++)
                    {
                        ...
                        {
                            Block.blocksList[j1].beginLeavesDecay(par1World, par2 + k, par3 + l, par4 + i1);
                        }
                    }
                }
            }
                    ...
                    for (int i1 = -byte0; i1 <= byte0; i1++)
                    {
                        for (int l1 = -byte0; l1 <= byte0; l1++)
                        {
                            for (int j2 = -byte0; j2 <= byte0; j2++)
                            {
                                ...
                                if (block != null && block.canSustainLeaves(par1World, par2 + i1, par3 + l1, par4 + j2))
                                {
                                    adjacentTreeBlocks[(i1 + l) * k + (l1 + l) * byte1 + j2 + l] = 0;
                                    continue;
                                }

                                if (block != null && block.isLeaves(par1World, par2 + i1, par3 + l1, par4 + j2))
                                {
    ...
    {
        return (par2 & 3) != 1 ? (par2 & 3) != 3 ? blockIndexInTexture : blockIndexInTexture + 144 : blockIndexInTexture + 80;
    }
    ...
    public ArrayList onSheared(ItemStack itemstack, World world, int i, int j, int k, int l)
    {
        ...
        arraylist.add(new ItemStack(this, 1, world.getBlockMetadata(i, j, k) & 3));
        return arraylist;
    }

    public void beginLeavesDecay(World world, int i, int j, int k)
    {
        world.setBlockMetadata(i, j, k, world.getBlockMetadata(i, j, k) | 8);
    }
