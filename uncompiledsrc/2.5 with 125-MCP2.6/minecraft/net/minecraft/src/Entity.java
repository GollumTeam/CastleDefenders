...
public abstract class Entity
{
    ...
    public void setAngles(float par1, float par2)
    {
        ...
        rotationYaw = (float)((double)rotationYaw + (double)par1 * 0.14999999999999999D);
        rotationPitch = (float)((double)rotationPitch - (double)par2 * 0.14999999999999999D);
    ...
    public void onEntityUpdate()
    {
        ...
        if (isSprinting() && !isInWater())
        {
            ...
            int i1 = MathHelper.floor_double(posY - 0.20000000298023224D - (double)yOffset);
            int i = MathHelper.floor_double(posZ);
            int j1 = worldObj.getBlockId(l, i1, i);
        ...
        if (handleWaterMovement())
        {
            if (!inWater && !firstUpdate)
            {
                ...
                for (int j = 0; (float)j < 1.0F + width * 20F; j++)
                {
                    ...
                    worldObj.spawnParticle("bubble", posX + (double)f4, f1 + 1.0F, posZ + (double)f2, motionX, motionY - (double)(rand.nextFloat() * 0.2F), motionZ);
                }

                for (int k = 0; (float)k < 1.0F + width * 20F; k++)
                {
                    float f5 = (rand.nextFloat() * 2.0F - 1.0F) * width;
                    float f3 = (rand.nextFloat() * 2.0F - 1.0F) * width;
                    worldObj.spawnParticle("splash", posX + (double)f5, f1 + 1.0F, posZ + (double)f3, motionX, motionY, motionZ);
                }
            }
    ...
    public boolean isOffsetPositionInLiquid(double par1, double par3, double par5)
    {
        ...
        return list.size() <= 0 ? !worldObj.isAnyLiquid(axisalignedbb) : false;
    }
    ...
    public void moveEntity(double par1, double par3, double par5)
    {
        ...
        else
        {
            ...
            if (stepHeight > 0.0F && flag1 && (flag || ySize < 0.05F) && (d2 != par1 || d4 != par5))
            {
                ...
                List list1 = worldObj.getCollidingBoundingBoxes(this, boundingBox.addCoord(d2, par3, d4));

                for (int l = 0; l < list1.size(); l++)
                {
                    par3 = ((AxisAlignedBB)list1.get(l)).calculateYOffset(boundingBox, par3);
                }
                ...
                {
                    par1 = ((AxisAlignedBB)list1.get(i1)).calculateXOffset(boundingBox, par1);
                }
                ...
                {
                    par5 = ((AxisAlignedBB)list1.get(j1)).calculateZOffset(boundingBox, par5);
                }
                ...
                else
                {
                    ...
                    {
                        par3 = ((AxisAlignedBB)list1.get(k1)).calculateYOffset(boundingBox, par3);
                    }

                    boundingBox.offset(0.0D, par3, 0.0D);
                }

                if (d8 * d8 + d10 * d10 >= par1 * par1 + par5 * par5)
                {
                    par1 = d8;
                    par3 = d6;
                    par5 = d10;
                ...
                else
                {
                    ...
                    if (d11 > 0.0D)
                    {
                        ySize = (float)((double)ySize + d11 + 0.01D);
                    }
                }
            }
            ...
            {
                distanceWalkedModified = (float)((double)distanceWalkedModified + (double)MathHelper.sqrt_double(d9 * d9 + d7 * d7) * 0.59999999999999998D);
                int l2 = MathHelper.floor_double(posX);
                int j2 = MathHelper.floor_double(posY - 0.20000000298023224D - (double)yOffset);
                int j3 = MathHelper.floor_double(posZ);
                int l1 = worldObj.getBlockId(l2, j2, j3);

                if (l1 == 0 && worldObj.getBlockId(l2, j2 - 1, j3) == Block.fence.blockID)
                {
                    l1 = worldObj.getBlockId(l2, j2 - 1, j3);
                }

                if (distanceWalkedModified > (float)nextStepDistance && l1 > 0)
                {
                    nextStepDistance = (int)distanceWalkedModified + 1;
                    playStepSound(l2, j2, j3, l1);
                    Block.blocksList[l1].onEntityWalking(worldObj, l2, j2, j3, this);
                }
            }

            int i3 = MathHelper.floor_double(boundingBox.minX + 0.001D);
            int k2 = MathHelper.floor_double(boundingBox.minY + 0.001D);
            int k3 = MathHelper.floor_double(boundingBox.minZ + 0.001D);
            int i2 = MathHelper.floor_double(boundingBox.maxX - 0.001D);
            int l3 = MathHelper.floor_double(boundingBox.maxY - 0.001D);
            int i4 = MathHelper.floor_double(boundingBox.maxZ - 0.001D);

            if (worldObj.checkChunksExist(i3, k2, k3, i2, l3, i4))
            {
                for (int j4 = i3; j4 <= i2; j4++)
                {
                    for (int k4 = k2; k4 <= l3; k4++)
                    {
                        for (int l4 = k3; l4 <= i4; l4++)
                        {
    ...
    protected NBTTagList newDoubleNBTList(double par1ArrayOfDouble[])
    {
        ...
        {
            double d = ad[j];
            nbttaglist.appendTag(new NBTTagDouble((String)null, d));
        }
    ...
    public void updateRidden()
    {
        ...
        else
        {
            ...
            if (ridingEntity != null)
            {
                ...
                rotationYaw = (float)((double)rotationYaw + d);
                rotationPitch = (float)((double)rotationPitch + d1);
            }
        }
    }
    ...
    public void mountEntity(Entity par1Entity)
    {
        ...
        else if (ridingEntity == par1Entity)
        {
            ridingEntity.riddenByEntity = null;
            ridingEntity = null;
    ...
    {
        return ridingEntity != null && ridingEntity.shouldRiderSit() || getFlag(2);
    }
