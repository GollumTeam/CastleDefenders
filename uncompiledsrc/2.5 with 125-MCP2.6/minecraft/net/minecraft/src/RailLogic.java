...
public class RailLogic
{
    ...
    public RailLogic(BlockRail par1BlockRail, World par2World, int par3, int par4, int par5)
    {
        ...
        BlockRail blockrail = (BlockRail)Block.blocksList[i];
        int j = blockrail.getBasicRailMetadata(par2World, null, par3, par4, par5);
        isPoweredRail = !blockrail.isFlexibleRail(par2World, par3, par4, par5);
        canMakeSlopes = blockrail.canMakeSlopes(par2World, par3, par4, par5);
        setConnections(j);
    }
    ...
    {
        return BlockRail.isRailBlockAt(worldObj, par1, par2, par3) ? true : BlockRail.isRailBlockAt(worldObj, par1, par2 + 1, par3) ? true : BlockRail.isRailBlockAt(worldObj, par1, par2 - 1, par3);
    }

    private RailLogic getMinecartTrackLogic(ChunkPosition par1ChunkPosition)
    {
        return BlockRail.isRailBlockAt(worldObj, par1ChunkPosition.x, par1ChunkPosition.y, par1ChunkPosition.z) ? new RailLogic(rail, worldObj, par1ChunkPosition.x, par1ChunkPosition.y, par1ChunkPosition.z) : BlockRail.isRailBlockAt(worldObj, par1ChunkPosition.x, par1ChunkPosition.y + 1, par1ChunkPosition.z) ? new RailLogic(rail, worldObj, par1ChunkPosition.x, par1ChunkPosition.y + 1, par1ChunkPosition.z) : BlockRail.isRailBlockAt(worldObj, par1ChunkPosition.x, par1ChunkPosition.y - 1, par1ChunkPosition.z) ? new RailLogic(rail, worldObj, par1ChunkPosition.x, par1ChunkPosition.y - 1, par1ChunkPosition.z) : null;
    }
