...
public class EntityRenderer
{
    ...
    private void orientCamera(float par1)
    {
        ...
        if (entityliving.isPlayerSleeping())
        {
            ...
            if (!mc.gameSettings.debugCamEnable)
            {
                ...
                if (block != null && block.isBed(mc.theWorld, i, j, k, entityliving))
                {
                    int l = block.getBedDirection(mc.theWorld, i, j, k);
                    GL11.glRotatef(l * 90, 0.0F, 1.0F, 0.0F);
                }
        ...
        else if (mc.gameSettings.thirdPersonView > 0)
        {
            ...
            if (mc.gameSettings.debugCamEnable)
            {
                float f3 = prevDebugCamYaw + (debugCamYaw - prevDebugCamYaw) * par1;
                float f1 = prevDebugCamPitch + (debugCamPitch - prevDebugCamPitch) * par1;
            ...
            else
            {
                ...
                double d4 = (double)(-MathHelper.sin((f4 / 180F) * (float)Math.PI) * MathHelper.cos((f2 / 180F) * (float)Math.PI)) * d3;
                double d5 = (double)(MathHelper.cos((f4 / 180F) * (float)Math.PI) * MathHelper.cos((f2 / 180F) * (float)Math.PI)) * d3;
                double d6 = (double)(-MathHelper.sin((f2 / 180F) * (float)Math.PI)) * d3;
                ...
                GL11.glRotatef(entityliving.rotationYaw - f4, 0.0F, 1.0F, 0.0F);
                GL11.glTranslatef(0.0F, 0.0F, (float)(-d3));
                GL11.glRotatef(f4 - entityliving.rotationYaw, 0.0F, 1.0F, 0.0F);
        ...
        for (int k = 0; k < 2; k++)
        {
            ...
            if (debugViewDirection == 0)
            {
                ...
                if (mc.objectMouseOver != null && entityliving.isInsideOfMaterial(Material.water) && (entityliving instanceof EntityPlayer) && !mc.gameSettings.hideGUI)
                {
                    ...
                    if (!ForgeHooksClient.onBlockHighlight(renderglobal, entityplayer, mc.objectMouseOver, 0, entityplayer.inventory.getCurrentItem(), par1))
                    {
            ...
            if (cameraZoom == 1.0D && (entityliving instanceof EntityPlayer) && !mc.gameSettings.hideGUI && mc.objectMouseOver != null && !entityliving.isInsideOfMaterial(Material.water))
            {
                ...
                if (!ForgeHooksClient.onBlockHighlight(renderglobal, entityplayer1, mc.objectMouseOver, 0, entityplayer1.inventory.getCurrentItem(), par1))
                {
            ...
            ForgeHooksClient.onRenderWorldLast(renderglobal, par1);
            ...
            for (int l1 = i1 - k1; l1 <= i1 + k1; l1++)
            {
                for (int i2 = k - k1; i2 <= k + k1; i2++)
                {
                    ...
                    if (world.getWorldChunkManager().getTemperatureAtHeight(f8, k2) >= 0.15F)
                    {
                        ...
                        double d3 = (double)((float)l1 + 0.5F) - entityliving.posZ;
                        float f13 = MathHelper.sqrt_double(d5 * d5 + d3 * d3) / (float)k1;
                        float f14 = 1.0F;
                    ...
                    double d4 = (double)((float)i2 + 0.5F) - entityliving.posX;
                    double d6 = (double)((float)l1 + 0.5F) - entityliving.posZ;
                    float f15 = MathHelper.sqrt_double(d4 * d4 + d6 * d6) / (float)k1;
                    float f16 = 1.0F;
    ...
    private void updateFogColor(float par1)
    {
        ...
        if (mc.gameSettings.renderDistance < 2)
        {
            ...
            float f4 = (float)entityliving.getLook(par1).dotProduct(vec3d2);
            ...
            if (f4 > 0.0F)
            {
                ...
                {
                    f4 *= af[3];
                    fogColorRed = fogColorRed * (1.0F - f4) + af[0] * f4;
                    fogColorGreen = fogColorGreen * (1.0F - f4) + af[1] * f4;
                    fogColorBlue = fogColorBlue * (1.0F - f4) + af[2] * f4;
                }
            }
        }
        ...
        float f7 = world.getRainStrength(par1);

        if (f7 > 0.0F)
        {
            float f5 = 1.0F - f7 * 0.5F;
            float f8 = 1.0F - f7 * 0.4F;
            fogColorRed *= f5;
            fogColorGreen *= f5;
            fogColorBlue *= f8;
        }

        float f6 = world.getWeightedThunderStrength(par1);
    ...
    private void setupFog(int par1, float par2)
    {
        ...
        else
        {
            ...
            else if (cloudFog)
            {
                ...
                if (mc.gameSettings.anaglyph)
                {
                    float f8 = (f1 * 30F + f18 * 59F + f5 * 11F) / 100F;
                    float f12 = (f1 * 30F + f18 * 70F) / 100F;
