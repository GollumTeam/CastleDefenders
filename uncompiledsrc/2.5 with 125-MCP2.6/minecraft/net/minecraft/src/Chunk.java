...
public class Chunk
{
    ...
    public Chunk(World par1World, byte par2ArrayOfByte[], int par3, int par4)
    {
        ...
        for (int j = 0; j < 16; j++)
        {
            for (int k = 0; k < 16; k++)
            {
                for (int l = 0; l < i; l++)
                {
                    int i1 = par2ArrayOfByte[j << 11 | k << 7 | l] & 0xff;
                    ...
                    {
                        storageArrays[j1] = new ExtendedBlockStorage(j1 << 4);
                    }

                    storageArrays[j1].setExtBlockID(j, l & 0xf, k, i1);
                }
            }
        }
    }

    public Chunk(World world, byte abyte0[], byte abyte1[], int i, int j)
    {
        this(world, i, j);
        int k = abyte0.length / 256;
        ...
        for (int l = 0; l < 16; l++)
        {
            for (int i1 = 0; i1 < 16; i1++)
            {
                for (int j1 = 0; j1 < k; j1++)
                {
                    ...
                    if (storageArrays[j2] == null)
                    {
                        storageArrays[j2] = new ExtendedBlockStorage(j2 << 4);
                    }

                    storageArrays[j2].setExtBlockID(l, j1 & 0xf, i1, l1);
                    storageArrays[j2].setExtBlockMetadata(l, j1 & 0xf, i1, i2);
                }
            }
        }
    }
    ...
    public void generateSkylightMap()
    {
        ...
        do
        {
            ...
            for (int l = 0; l < 16; l++)
            {
                ...
                do
                {
                    k1 -= getBlockLightOpacity(j, l1, l);

                    if (k1 > 0)
                    {
                        ExtendedBlockStorage extendedblockstorage = storageArrays[l1 >> 4];

                        if (extendedblockstorage != null)
                        {
                            extendedblockstorage.setExtSkylightValue(j, l1 & 0xf, l, k1);
                            worldObj.func_48464_p((xPosition << 4) + j, l1, (zPosition << 4) + l);
                        }
                    }
                }
                while (--l1 > 0 && k1 > 0);
    ...
    public int getBlockID(int par1, int par2, int par3)
    {
        ...
        else
        {
            ...
            return extendedblockstorage == null ? 0 : extendedblockstorage.getExtBlockID(par1, par2 & 0xf, par3);
        }
    }
    ...
    public int getBlockMetadata(int par1, int par2, int par3)
    {
        ...
        {
            ExtendedBlockStorage extendedblockstorage = storageArrays[par2 >> 4];
            return extendedblockstorage == null ? 0 : extendedblockstorage.getExtBlockMetadata(par1, par2 & 0xf, par3);
        }
    }
    ...
    public boolean setBlockIDWithMetadata(int par1, int par2, int par3, int par4, int par5)
    {
        ...
        if (k != 0)
        {
            ...
            if (Block.blocksList[k] != null && Block.blocksList[k].hasTileEntity(getBlockMetadata(par1, par2, par3)))
            {
        ...
        if (par4 != 0)
        {
            ...
            if (Block.blocksList[par4] != null && Block.blocksList[par4].hasTileEntity(par5))
            {
                ...
                if (tileentity == null)
                {
                    tileentity = Block.blocksList[par4].getTileEntity(par5);
    ...
    {
        ExtendedBlockStorage extendedblockstorage = par2 >> 4 < storageArrays.length && par2 >> 4 >= 0 ? storageArrays[par2 >> 4] : null;
        ...
        if (j > 0 && Block.blocksList[j] != null && Block.blocksList[j].hasTileEntity(extendedblockstorage.getExtBlockMetadata(par1, par2 & 0xf, par3)))
        {
            TileEntity tileentity = getChunkBlockTileEntity(par1, par2, par3);
    ...
    {
        ExtendedBlockStorage extendedblockstorage = par3 >> 4 < storageArrays.length && par3 >> 4 >= 0 ? storageArrays[par3 >> 4] : null;
        return extendedblockstorage != null ? par1EnumSkyBlock != EnumSkyBlock.Sky ? par1EnumSkyBlock != EnumSkyBlock.Block ? par1EnumSkyBlock.defaultLightValue : extendedblockstorage.getExtBlocklightValue(par2, par3 & 0xf, par4) : extendedblockstorage.getExtSkylightValue(par2, par3 & 0xf, par4) : par1EnumSkyBlock.defaultLightValue;
    }
    ...
    public int getBlockLightValue(int par1, int par2, int par3, int par4)
    {
        ...
        if (extendedblockstorage == null)
        {
            return worldObj.worldProvider.hasNoSky || par4 >= EnumSkyBlock.Sky.defaultLightValue ? 0 : EnumSkyBlock.Sky.defaultLightValue - par4;
        }
    ...
    public TileEntity getChunkBlockTileEntity(int par1, int par2, int par3)
    {
        ...
        if (tileentity == null)
        {
            ...
            if (i <= 0 || Block.blocksList[i] == null || !Block.blocksList[i].hasTileEntity(j))
            {
                return null;
            }
            ...
            if (tileentity == null)
            {
                tileentity = Block.blocksList[i].getTileEntity(j);
    ...
    public void setChunkBlockTileEntity(int par1, int par2, int par3, TileEntity par4TileEntity)
    {
        ...
        if (i > 0 && Block.blocksList[i] != null && Block.blocksList[i].hasTileEntity(getBlockMetadata(par1, par2, par3)))
        {
            TileEntity tileentity = (TileEntity)chunkTileEntityMap.get(chunkposition);
    ...
    {
        int i = MathHelper.floor_double((par2AxisAlignedBB.minY - World.MAX_ENTITY_RADIUS) / 16D);
        int j = MathHelper.floor_double((par2AxisAlignedBB.maxY + World.MAX_ENTITY_RADIUS) / 16D);
    ...
    public void func_48494_a(byte par1ArrayOfByte[], int par2, int par3, boolean par4)
    {
        ...
        for (Iterator iterator = chunkTileEntityMap.values().iterator(); iterator.hasNext(); tileentity.getBlockType())
        {
            tileentity = (TileEntity)iterator.next();
            tileentity.updateContainingBlockInfo();
        ...
        if (par4)
        {
            ...
            int l1 = i + blockBiomeArray.length;
        }
        ...
        for (Iterator iterator1 = chunkTileEntityMap.values().iterator(); iterator1.hasNext(); tileentity1.updateContainingBlockInfo())
        {
            tileentity1 = (TileEntity)iterator1.next();
            int i2 = tileentity1.xCoord & 0xf;
            int j2 = tileentity1.yCoord;
            int k2 = tileentity1.zCoord & 0xf;
            Block block = tileentity1.getBlockType();

            if (block == null || block.blockID != getBlockID(i2, j2, k2) || tileentity1.getBlockMetadata() != getBlockMetadata(i2, j2, k2))
            {
        ...
        for (Iterator iterator2 = arraylist.iterator(); iterator2.hasNext(); tileentity2.invalidate())
        {
            tileentity2 = (TileEntity)iterator2.next();
        }
    }
    ...
    public BiomeGenBase func_48490_a(int par1, int par2, WorldChunkManager par3WorldChunkManager)
    {
        ...
        return BiomeGenBase.biomeList[i] != null ? BiomeGenBase.biomeList[i] : BiomeGenBase.plains;
    }
