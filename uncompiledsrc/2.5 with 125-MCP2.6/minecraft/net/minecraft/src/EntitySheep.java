...
public class EntitySheep extends EntityAnimal implements IShearable
{
    ...
    {
        return sheepTimer > 0 ? sheepTimer < 4 || sheepTimer > 36 ? sheepTimer >= 4 ? -((float)(sheepTimer - 40) - par1) / 4F : ((float)sheepTimer - par1) / 4F : 1.0F : 0.0F;
    }
    ...
    public float func_44002_d(float par1)
    {
        ...
        {
            return sheepTimer <= 0 ? rotationPitch / (180F / (float)Math.PI) : ((float)Math.PI / 5F);
        }
    }
    ...
    public static int getRandomFleeceColor(Random par0Random)
    {
        ...
        return i >= 5 ? i >= 10 ? i >= 15 ? i >= 18 ? ((byte)(par0Random.nextInt(500) != 0 ? 0 : 6)) : 12 : 8 : 7 : 15;
    }
    ...
    public ArrayList onSheared(ItemStack itemstack, World world, int i, int j, int k, int l)
    {
        ...
        setSheared(true);
        int i1 = 1 + rand.nextInt(3);

        for (int j1 = 0; j1 < i1; j1++)
        {
            arraylist.add(new ItemStack(Block.cloth.blockID, 1, getFleeceColor()));
        }
