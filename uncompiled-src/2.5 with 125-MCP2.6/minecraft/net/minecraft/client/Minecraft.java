...
public abstract class Minecraft implements Runnable
{
    ...
    private void runGameLoop()
    {
        ...
        else
        {
            ...
            if (!skipRenderWorld)
            {
                ...
                FMLClientHandler.instance().onRenderTickStart(timer.renderPartialTicks);
                Profiler.endStartSection("gameRenderer");
                entityRenderer.updateCameraAndRender(timer.renderPartialTicks);
                Profiler.endSection();
                FMLClientHandler.instance().onRenderTickEnd(timer.renderPartialTicks);
            }
    ...
    private void updateDebugProfilerName(int par1)
    {
        ...
        if (list != null && list.size() != 0)
        {
            ...
            else if (--par1 < list.size() && !((ProfilerResult)list.get(par1)).name.equals("unspecified"))
            {
                ...
                {
                    debugProfilerName = (new StringBuilder()).append(debugProfilerName).append(".").toString();
                }

                debugProfilerName = (new StringBuilder()).append(debugProfilerName).append(((ProfilerResult)list.get(par1)).name).toString();
            }
        }
    }
    ...
    private void displayDebugInfo(long par1)
    {
        ...
        for (int j2 = 0; j2 < frameTimes.length; j2++)
        {
            int i1 = ((j2 - numRecordedFrameTimes & frameTimes.length - 1) * 255) / frameTimes.length;
            int k1 = (i1 * i1) / 255;
            k1 = (k1 * k1) / 255;
            int i3 = (k1 * k1) / 255;
            i3 = (i3 * i3) / 255;
            ...
            long l3 = frameTimes[j2] / 0x30d40L;
            long l4 = tickTimes[j2] / 0x30d40L;
            tessellator.addVertex((float)j2 + 0.5F, (float)((long)displayHeight - l3) + 0.5F, 0.0D);
            tessellator.addVertex((float)j2 + 0.5F, (float)displayHeight + 0.5F, 0.0D);
            tessellator.setColorOpaque_I(0xff000000 + k1 * 0x10000 + k1 * 256 + k1 * 1);
            tessellator.addVertex((float)j2 + 0.5F, (float)((long)displayHeight - l3) + 0.5F, 0.0D);
            tessellator.addVertex((float)j2 + 0.5F, (float)((long)displayHeight - (l3 - l4)) + 0.5F, 0.0D);
        }

        tessellator.draw();
        int k2 = 160;
        int j1 = displayWidth - k2 - 10;
        int i2 = displayHeight - k2 * 2;
        GL11.glEnable(GL11.GL_BLEND);
        tessellator.startDrawingQuads();
        tessellator.setColorRGBA_I(0, 200);
        tessellator.addVertex((float)j1 - (float)k2 * 1.1F, (float)i2 - (float)k2 * 0.6F - 16F, 0.0D);
        tessellator.addVertex((float)j1 - (float)k2 * 1.1F, i2 + k2 * 2, 0.0D);
        tessellator.addVertex((float)j1 + (float)k2 * 1.1F, i2 + k2 * 2, 0.0D);
        tessellator.addVertex((float)j1 + (float)k2 * 1.1F, (float)i2 - (float)k2 * 0.6F - 16F, 0.0D);
        tessellator.draw();
        ...
        {
            ProfilerResult profilerresult1 = (ProfilerResult)list.get(i4);
            int j3 = MathHelper.floor_double(profilerresult1.sectionPercentage / 4D) + 1;
            tessellator.startDrawing(6);
            tessellator.setColorOpaque_I(profilerresult1.getDisplayColor());
            tessellator.addVertex(j1, i2, 0.0D);

            for (int k4 = j3; k4 >= 0; k4--)
            {
                float f = (float)(((d + (profilerresult1.sectionPercentage * (double)k4) / (double)j3) * Math.PI * 2D) / 100D);
                float f4 = MathHelper.sin(f) * (float)k2;
                float f2 = MathHelper.cos(f) * (float)k2 * 0.5F;
                tessellator.addVertex((float)j1 + f4, (float)i2 - f2, 0.0D);
            }
            ...
            for (int i5 = j3; i5 >= 0; i5--)
            {
                float f1 = (float)(((d + (profilerresult1.sectionPercentage * (double)i5) / (double)j3) * Math.PI * 2D) / 100D);
                float f5 = MathHelper.sin(f1) * (float)k2;
                float f3 = MathHelper.cos(f1) * (float)k2 * 0.5F;
                tessellator.addVertex((float)j1 + f5, (float)i2 - f3, 0.0D);
                tessellator.addVertex((float)j1 + f5, ((float)i2 - f3) + 10F, 0.0D);
            }
        ...
        int k3 = 0xffffff;
        fontRenderer.drawStringWithShadow(s, j1 - k2, i2 - k2 / 2 - 16, k3);
        fontRenderer.drawStringWithShadow(s = (new StringBuilder()).append(decimalformat.format(profilerresult.globalPercentage)).append("%").toString(), (j1 + k2) - fontRenderer.getStringWidth(s), i2 - k2 / 2 - 16, k3);

        for (int j4 = 0; j4 < list.size(); j4++)
        {
            ProfilerResult profilerresult2 = (ProfilerResult)list.get(j4);
            String s1 = "";

            if (!profilerresult2.name.equals("unspecified"))
            {
                s1 = (new StringBuilder()).append(s1).append("[").append(j4 + 1).append("] ").toString();
            }
            else
            {
            ...
            fontRenderer.drawStringWithShadow(s1, j1 - k2, i2 + k2 / 2 + j4 * 8 + 20, profilerresult2.getDisplayColor());
            fontRenderer.drawStringWithShadow(s1 = (new StringBuilder()).append(decimalformat.format(profilerresult2.sectionPercentage)).append("%").toString(), (j1 + k2) - 50 - fontRenderer.getStringWidth(s1), i2 + k2 / 2 + j4 * 8 + 20, profilerresult2.getDisplayColor());
            fontRenderer.drawStringWithShadow(s1 = (new StringBuilder()).append(decimalformat.format(profilerresult2.globalPercentage)).append("%").toString(), (j1 + k2) - fontRenderer.getStringWidth(s1), i2 + k2 / 2 + j4 * 8 + 20, profilerresult2.getDisplayColor());
        }
    }
        ...
        if (par1 != 0 || leftClickCounter <= 0)
        {
            ...
            else if (objectMouseOver.typeOfHit == EnumMovingObjectType.TILE)
            {
                ...
                else
                {
                    ...
                    if (playerController.onPlayerRightClick(thePlayer, theWorld, itemstack, i, j, k, l))
                    {
                        flag = false;
                        thePlayer.swingItem();
                    }
    ...
    public void runTick()
    {
        ...
        if (thePlayer != null)
        {
            ...
            if (ichunkprovider instanceof ChunkProviderLoadOrGenerate)
            {
                ...
                chunkproviderloadorgenerate.setCurrentChunkOver(i, j1);
            }
        }
                ...
                if (l <= 200L)
                {
                    ...
                    if (j != 0)
                    {
                        ...
                        if (gameSettings.noclip)
                        {
                            ...
                            gameSettings.noclipRate += (float)j * 0.25F;
                        }
                    }
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    else
                    {
                        ...
                        for (int k = 0; k < 9; k++)
                        {
                            ...
                            {
                                thePlayer.inventory.currentItem = k;
                            }
                        }
    ...
    public void usePortal(int i, Teleporter teleporter)
    {
        ...
        WorldProvider worldprovider = WorldProvider.getProviderForDimension(i);
        WorldProvider worldprovider1 = WorldProvider.getProviderForDimension(j);
        double d = thePlayer.posX;
        double d1 = thePlayer.posZ;
        double d2 = worldprovider1.getMovementFactor() / worldprovider.getMovementFactor();
        d *= d2;
        d1 *= d2;
        World world = new World(theWorld, worldprovider);
        ...
        {
            changeWorld(world, worldprovider1.getDepartMessage(), thePlayer);
        }
        else
        {
            changeWorld(world, worldprovider.getWelcomeMessage(), thePlayer);
        }
    ...
    {
        theWorld = null;
        changeWorld2((World)null, par1Str);
    }
    ...
    {
        changeWorld(par1World, par2Str, (EntityPlayer)null);
    }
    ...
    public void respawn(boolean par1, int par2, boolean par3)
    {
        ...
        if (!theWorld.isRemote && theWorld.worldProvider.canRespawnHere())
        {
            par2 = theWorld.worldProvider.worldType;
        }
    ...
    public static void startMainThread(String par0Str, String par1Str, String par2Str)
    {
        ...
        {
            minecraftimpl.session = new Session(par0Str, par1Str);
        }
        else
        {
    ...
    {
        return (thePlayer instanceof EntityClientPlayerMP) ? ((EntityClientPlayerMP)thePlayer).sendQueue : null;
    }
    ...
    public static void main(String par0ArrayOfStr[])
    {
        ...
        FMLClientHandler.instance().preGameLoad(s, s1);
    }

    public static void fmlReentry(String s, String s1)
    {
    ...
    public static void func_52004_D()
    {
        ...
        playerusagesnooper.func_52022_a("client_brand", "forge");
