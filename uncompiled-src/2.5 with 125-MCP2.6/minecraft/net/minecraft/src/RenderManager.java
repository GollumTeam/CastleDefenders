...
public class RenderManager
{
    ...
    public void cacheActiveRenderInfo(World par1World, RenderEngine par2RenderEngine, FontRenderer par3FontRenderer, EntityLiving par4EntityLiving, GameSettings par5GameSettings, float par6)
    {
        ...
        {
            int i = MathHelper.floor_double(par4EntityLiving.posX);
            int j = MathHelper.floor_double(par4EntityLiving.posY);
            int k = MathHelper.floor_double(par4EntityLiving.posZ);
            Block block = Block.blocksList[par1World.getBlockId(i, j, k)];

            if (block != null && block.isBed(par1World, i, j, k, par4EntityLiving))
            {
                int l = block.getBedDirection(par1World, i, j, k);
                playerViewY = l * 90 + 180;
                playerViewX = 0.0F;
            }
        }
        else
