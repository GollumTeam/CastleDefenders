...
public class Explosion
{
    ...
    public void doExplosionA()
    {
        ...
        for (int j = 0; j < i; j++)
        {
            for (int l = 0; l < i; l++)
            {
                ...
                for (int j1 = 0; j1 < i; j1++)
                {
                    ...
                    double d6 = ((float)j / ((float)i - 1.0F)) * 2.0F - 1.0F;
                    double d7 = ((float)l / ((float)i - 1.0F)) * 2.0F - 1.0F;
                    double d8 = ((float)j1 / ((float)i - 1.0F)) * 2.0F - 1.0F;
                    ...
                    double d = explosionX;
                    double d2 = explosionY;
                    double d4 = explosionZ;
                    float f2 = 0.3F;
                    ...
                    do
                    {
                        ...
                        int l2 = MathHelper.floor_double(d);
                        int i3 = MathHelper.floor_double(d2);
                        int j3 = MathHelper.floor_double(d4);
                        ...
                        if (k3 > 0)
                        {
                            ...
                            {
                                ISpecialResistance ispecialresistance = (ISpecialResistance)Block.blocksList[k3];
                                f1 -= (ispecialresistance.getSpecialExplosionResistance(worldObj, l2, i3, j3, explosionX, explosionY, explosionZ, exploder) + 0.3F) * f2;
                            }
                            else
                            {
        ...
        for (int k2 = 0; k2 < list.size(); k2++)
        {
            ...
            double d10 = entity.getDistance(explosionX, explosionY, explosionZ) / (double)explosionSize;

            if (d10 <= 1.0D)
            {
                double d1 = entity.posX - explosionX;
                double d3 = entity.posY - explosionY;
                double d5 = entity.posZ - explosionZ;
                double d11 = MathHelper.sqrt_double(d1 * d1 + d3 * d3 + d5 * d5);
                d1 /= d11;
                d3 /= d11;
                d5 /= d11;
                ...
                entity.motionX += d1 * d13;
                entity.motionY += d3 * d13;
                entity.motionZ += d5 * d13;
            }
        }
