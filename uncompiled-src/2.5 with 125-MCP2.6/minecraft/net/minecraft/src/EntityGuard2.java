...
public class EntityGuard2 extends EntityTameable
{
    ...
    private float timeGuard2IsShaking;
    private float prevTimeGuard2IsShaking;
    private static final ItemStack defaultHeldItem;
    protected int attackStrength;

    public EntityGuard2(World world)
    {
        super(world);
    ...
    public boolean getCanSpawnHere()
    {
        ...
        else
        {
            ...
            List list = worldObj.getEntitiesWithinAABB(net.minecraft.src.EntityGuard2.class, AxisAlignedBB.getBoundingBoxFromPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(2D, 2D, 2D));
            return worldObj.getBlockId(i, j - 1, k) == 233 && worldObj.checkIfAABBIsClear(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox).size() == 0 && list.isEmpty();
        }
    }
    ...
    public void onLivingUpdate()
    {
        ...
        {
            field_25052_g = true;
            timeGuard2IsShaking = 0.0F;
            prevTimeGuard2IsShaking = 0.0F;
    ...
    public void onUpdate()
    {
        ...
        {
            if (timeGuard2IsShaking == 0.0F)
            {
                worldObj.playSoundAtEntity(this, "mob.Guard2.shake", getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
            }

            prevTimeGuard2IsShaking = timeGuard2IsShaking;
            timeGuard2IsShaking += 0.05F;

            if (prevTimeGuard2IsShaking >= 2.0F)
            {
                isShaking = false;
                field_25052_g = false;
                prevTimeGuard2IsShaking = 0.0F;
            ...
            if (timeGuard2IsShaking > 0.4F)
            {
                float f = (float)boundingBox.minY;
                int i = (int)(MathHelper.sin((timeGuard2IsShaking - 0.4F) * (float)Math.PI) * 7F);
    ...
    public boolean getGuard2Shaking()
    {
        return isShaking;
    }
    ...
    {
        EntityGuard2 entityguard2 = new EntityGuard2(worldObj);
        entityguard2.setOwner(getOwnerName());
        entityguard2.setTamed(true);
        return entityguard2;
    }
    ...
    public boolean func_48135_b(EntityAnimal entityanimal)
    {
        ...
        if (!(entityanimal instanceof EntityGuard2))
        {
            return false;
        }

        EntityGuard2 entityguard2 = (EntityGuard2)entityanimal;

        if (!entityguard2.isTamed())
        {
            return false;
        }

        if (entityguard2.isSitting())
        {
            return false;
        }
        else
        {
            return isInLove() && entityguard2.isInLove();
        }
    }
